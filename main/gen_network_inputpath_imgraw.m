...
function [layers, net_output_info, dag_net]=gen_network_inputpath_imgraw(inputpath_config)


dag_net=dagnn.DagNN();
start_var_name='data_input';

init_lr=inputpath_config.lr_factor;
layer_gen_info=[];
layer_gen_info.one_output_dim=3;
layer_gen_info.one_outputs={start_var_name};
layer_gen_info.init_lr=init_lr;

layer_gen_info=do_add_img_input_scratch_path(dag_net, inputpath_config, layer_gen_info);
...
function layer_gen_info=do_add_img_input_scratch_path(dag_net, inputpath_config, layer_gen_info)

layer_gen_info=add_scratch_img_input_block(dag_net, inputpath_config, layer_gen_info);

conv_num=inputpath_config.conv_num_one_block;
conv_block_num=inputpath_config.conv_block_num;
for b_idx=1:conv_block_num
    
    one_output_dim=inputpath_config.filter_num_blocks(b_idx);
...
function layer_gen_info=add_scratch_img_input_block(dag_net, inputpath_config, layer_gen_info)

init_lr=layer_gen_info.init_lr;
input_dim=layer_gen_info.one_output_dim;
one_output_dim=inputpath_config.filter_num_blocks(1);
...
[one_outputs_dimred, layer_name_dimred]=My_net_util.add_dim_reduce_layer(dag_net, one_outputs_dimred{1}, feat_dim_before, feat_dim_after, init_lr);
