function output_info=move_output_info_gpu(output_info)
...
if output_info.is_group_data
    ...
    for g_idx=1:child_num
        data_child_groups{g_idx}=move_output_info_gpu(data_child_groups{g_idx});
    end
    ...
    if isfield(output_info, 'x')
        output_info.x=move_to_gpu_values(output_info.x);
    end
    if isfield(output_info, 'aux')
        output_info.aux=move_to_gpu_values(output_info.aux);
    end
    if isfield(output_info, 'dzdx')
        output_info.dzdx=move_to_gpu_values(output_info.dzdx);
    end
    if isfield(output_info, 'dzdw')
        output_info.dzdw=move_to_gpu_values(output_info.dzdw);
    end
