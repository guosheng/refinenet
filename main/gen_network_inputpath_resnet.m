...
function [layers, net_output_info, dag_net]=gen_network_inputpath_resnet(inputpath_config)


pre_trained_model_file=inputpath_config.pre_trained_model_file;
...
reduce_stride_resnet(dag_net, inputpath_config.reduce_layer_stride_topk);
   
net_output_info=gen_net_output_info_resnet(dag_net, inputpath_config);
...
function reduce_stride_resnet(dag_net, topk)

    % for the resnet, it has two branch, so should times 2
    topk=2*topk;
    ...
    done_count=0;
    for l_idx=length(dag_net.layers):-1:1
        l=dag_net.layers(l_idx);
        block=l.block;
%         isconv=~isempty(strfind(l.name, 'conv'));
%         ispool=~isempty(strfind(l.name, 'pool'));
%         if  isconv||ispool 
        if isprop(block, 'stride')
            if any(block.stride~=1)
                block.stride=[1 1];
                
                fprintf('fix stride for layer:/n');
                disp(l);
                
                % for the object, seems unecesary!
%                 l.block=block;
...
function net_output_info=gen_net_output_info_resnet(dag_net, inputpath_config)


init_stage_idx=inputpath_config.init_output_level_idx;
output_level_num=inputpath_config.output_level_num;

tmp_net_output_info=gen_stage_output_info(dag_net, inputpath_config);
...
net_output_info=[];
net_output_info.var_names=var_names(1:output_level_num);
net_output_info.var_dims=valid_output_dim_vars(1:output_level_num);
...
function net_output_info=gen_stage_output_info(dag_net, train_opts)
    ...
    % the first RELU is the downsample layer
    ...
    % skip this stage:
%     output_dim_vars(start_v_idx)=first_var_dim;
%     stage_var_flags(start_v_idx)=true;
...
%                 assert(isa(input_layers{1}.block, 'dagnn.ReLU') || isa(input_layers{1}.block, 'dagnn.Pooling'));
    ...
    % notes: don't use the var idx, use the var name, that's more reliable
    net_output_info=[];
    net_output_info.var_names=var_names;
...
% start_var_name=dag_net.getOutputs();
% assert(length(start_var_name)==1);
% start_var_name=start_var_name{1};
...
% assert(length(start_var_name)==1);
% start_var_name=start_var_name{1};
% assert(strcmp(start_var_name, tmp_var_name));
